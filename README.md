# Textile Robotic Interaction Dataset for Designer Robot Collaboration HRI 2024 (July 2023)

This repository stores the scripts and text-based data for the collaborative experiments between two research projects, ***Project 1*** and ***Project 2***.

There were two robotic experiments.  Both had the general aim of scanning and recording image-based / video data of textile materials.  The set of materials used in these experiments was provided by the ***Project 1***.  They are 12 novel sustainable textile materials sourced and exhibited from an international novel biomaterials expo.  ***Project 2*** provided access to UFactory xArm7 robot arms at the human-robotic collaboration lab.

*A sample selection of data is available in the ```data``` directory.*  The full data is available as a .zip from ***anonmised for review***  Un-zip this directory and replace the ```data``` directory.  

A research paper is available at ***link to submitted research paper***, which
describes the set-up to the experiments.  This document describes technical
details, such as file formats and the scripts used to run the robot experiments.
